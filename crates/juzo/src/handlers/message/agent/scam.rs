...
pub async fn add(
    ...
    bot.send(
        JuzoAnswer::message(&message)
            .text(format!("{0} {{}} занесён в «Juzo | Scam System»", smail_tick(true))),
    )
    .await?;
...
pub async fn delete(
    ...
    match res {
        Ok(r) if r.rows_affected > 0 => {
            bot.send(
                JuzoAnswer::message(&message)
                    .text(format!("{0} {{}} вынесен из «Juzo | Scam System»", smail_tick(true))),
            )
            .await?;
        }
        _ => {
            bot.send(
                JuzoAnswer::message(&message)
                    .text(format!("{0} {{}} не находится в базе скама.", smail_pensil(true))),
            )
            .await?;
        }
    }
